...
class Family:
	...
	def _realign(self, read, cigarOffset, windowBuffer=200):
		...
		if read.reference_name != refName or readWindowStart < refStart or readWindowEnd > refEnd:
			...
			refWindow = refGenome[refName][refStart:refEnd]
		refAlignment = alignment.StripedSmithWaterman(query_sequence=refWindow.seq, gap_extend_penalty=2, gap_open_penalty=5, match_score=3, mismatch_score=-4)
	...
	def cigarToList(self, cigarTuples, isRead1, counterIncremeneted=False):
		...
		INDELs, inrement the abnormal read counter, so we can realign any INDELs later on
		"""
		cigarList = []
	...
	def consensus(self, abnormalityThresold=0.4):
		...
		self.R2start = [self.R2start[0] + R2softClip - self.R2startOffset]  # Compensate for any changes in soft-clipping
	...
	def _consensusByRead(self, sequences, qualities, cigars):
		...
		while True:
			...
			cigarOp = {"A": {0: 0, 3: 0, 4: 0, 5: 0}, "C": {0: 0, 3: 0, 4: 0, 5: 0}, "G": {0: 0, 3: 0, 4: 0, 5: 0}, "T": {0: 0, 3: 0, 4: 0, 5: 0}}
...
class Position:
	...
	def collapse(self, collapseIndices, collapseThreshold=3):
		...
		barcodesByFrequency = sortedcontainers.SortedListWithKey(list(self.plusFamilies.keys()), key=lambda x: self.plusFamilies[x].size)
		...
		# barcode occurs
		barcodesInFamilies = collections.OrderedDict()  # Store the base barcode of the family, as well as all barcodes which should be collapsed into that family
		barcodesByFrequency = sortedcontainers.SortedListWithKey(list(self.negFamilies.keys()), key=lambda x: self.negFamilies[x].size)
		for barcode in reversed(barcodesByFrequency):
		...
		for familyName, members in barcodesInFamilies.items():
			...
			for member in members:
				...
				# If this family has multiple members, indicate that all members of this family have been aggregared together
...
class FamilyCoordinator:
	...
	def __init__(self, inputFile, reference, familyIndices, familyThreshold, duplexIndices, duplexThreshold,
				barcodeLength, targets=None, baseBuffer=400, padding=10):
		...
		try:
			with open(targetFile) as f:
				for line in f:
					...
					if start > end:
						sys.stderr.write("ERROR: The start position \'%s\' is greater than the end position \'%s\'\n" % (start, end))
						exit(1)
	...
	def __iter__(self):
		...
		try:
			while True:
				...
				# one of the existing read's positions was processed a long time ago
				if pair.isSplit:
					continue
...
def validateArgs(args):
	...
	parser.add_argument("-c", "--config", type=lambda x: isValidFile(x, parser), help="An optional configuration file, which can provide one or more arguments")
	parser.add_argument("-i", "--input", type=lambda x: isValidFile(x, parser, True), required=True, help="Input sorted BAM file (use \"-\" to read from stdin [use control + d to stop reading])")
	parser.add_argument("-o", "--output", required=True, help="Output BAM file (use \"-\" to write to stdout). Will be unsorted")
	parser.add_argument("-fm", "--family_mask", required=True, type=str, help="Positions in the barcode to consider when collapsing reads into a consensus (1=Consider, 0=Ignore)")
	parser.add_argument("-dm", "--duplex_mask", required=True, type=str, help="Positions in the barcode to consider when determining if two families are in duplex (1=Consider, 0=Ignore)")
	parser.add_argument("-fmm", "--family_mismatch", required=True, type=int, help="Maximum number of mismatches permitted when collapsing reads into a family")
	parser.add_argument("-dmm", "--duplex_mismatch", required=True, type=int, help="Maximum number of mismatches permitted when identifying of two families are in duplex")
	parser.add_argument("-r", "--reference", required=True, type=lambda x: isValidFile(x, parser), help="Reference genome, in FASTA format")
	parser.add_argument("-t", "--targets", type=lambda x: isValidFile(x, parser), help="A BED file listing targets of interest")
...
parser = argparse.ArgumentParser(description="Collapsed duplicate sequences into a consensus")
parser.add_argument("-c", "--config", type=lambda x: isValidFile(x, parser), help="An optional configuration file, which can provide one or more arguments")
parser.add_argument("-i", "--input", type=lambda x: isValidFile(x, parser, True), help="Input sorted BAM file (use \"-\" to read from stdin [use control + d to stop reading])")
parser.add_argument("-o", "--output", help="Output BAM file (use \"-\" to write to stdout). Will be unsorted")
parser.add_argument("-fm", "--family_mask", type=str, help="Positions in the barcode to consider when collapsing reads into a consensus (1=Consider, 0=Ignore)")
parser.add_argument("-dm", "--duplex_mask", type=str, help="Positions in the barcode to consider when determining if two families are in duplex (1=Consider, 0=Ignore)")
parser.add_argument("-fmm", "--family_mismatch", type=int, help="Maximum number of mismatches permitted when collapsing reads into a family")
parser.add_argument("-dmm", "--duplex_mismatch", type=int, help="Maximum number of mismatches permitted when identifying of two families are in duplex")
parser.add_argument("-r", "--reference", type=lambda x: isValidFile(x, parser), help="Reference genome, in FASTA format")
parser.add_argument("-t", "--targets", type=lambda x: isValidFile(x, parser), help="A BED file listing targets of interest")
...
def main(args=None, sysStdin=None):
	...
	if args["config"] is not None:
		...
		try:
			...
			sys.stderr.write("ERROR: Unable to locate a section named \'collapse\' in the config file \'%s\'\n" % (args["config"]))
			exit(1)
	...
	if args["family_mismatch"] < 0:
		...
		raise parser.error("The lengths of \'-fm\--family_mask\' and \'-dm\--duplex_mask\' must be the same, because the barcode size is the same!")
	...
	readProcessor = FamilyCoordinator(inBAM, args["reference"], familyIndices, args["family_mismatch"],
									duplexIndices, args["duplex_mismatch"], len(args["duplex_mask"]) * 2, args["targets"])
