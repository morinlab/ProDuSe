...
def validateArgs(args):
	...
	parser.add_argument("-i", "--input", metavar="FASTQ", required=True, nargs=2, type=lambda x: isValidFile(x, parser), help="A pair of FASTQ files generated from paired-end sequencing. May be gzipped")
	parser.add_argument("-o", "--output", metavar="FASTQ", required=True, nargs=2, help="A pair of output FASTQ files, to which the trimmed reads will be written to. May be gzipped")
...
parser = argparse.ArgumentParser(description="Trims degenerate barcodes")
parser.add_argument("-c", "--config", metavar="INI", type=lambda x: isValidFile(x, parser), help="An optional configuration file, which can provide one or more arguments")
parser.add_argument("-i", "--input", metavar="FASTQ", nargs=2, type=lambda x: isValidFile(x, parser), help="A pair of FASTQ files generated from paired-end sequencing. May be gzipped")
parser.add_argument("-o", "--output", metavar="FASTQ", nargs=2, help="A pair of output FASTQ files, to which the trimmed reads will be written to. May be gzipped")
...
def main(args=None, sysStdin=None):
	...
	if args["config"] is not None:
		...
		try:
			...
			sys.stderr.write("ERROR: Unable to locate a section named \'trim\' in the config file \'%s\'\n" % (args["config"]))
			exit(1)
